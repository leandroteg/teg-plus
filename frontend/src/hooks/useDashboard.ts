...
// Mapeia a resposta do RPC (nomes de campo diferentes) para DashboardData
...
// Fallback: monta dashboard diretamente das tabelas caso o RPC falhe
async function fetchDashboardDireto(periodo: string): Promise<DashboardData> {
  ...
  const [reqRes, aprRes] = await Promise.all([
    ...
    supabase
      .from('aprovacoes')
      .select('id, requisicao_id, aprovador_nome, aprovador_email, nivel, status, token, data_limite')
      .eq('status', 'pendente')
      .order('created_at', { ascending: false })
      .limit(10),
  ])
  ...
  const aprovacoes = (aprRes.data ?? []) as Aprovacao[]
  ...
  const por_status = Object.entries(porStatusMap).map(([status, v]) => ({ status, ...v }))
  ...
  const por_obra = Object.entries(porObraMap)
    .map(([obra_nome, v]) => ({ obra_nome, ...v }))
