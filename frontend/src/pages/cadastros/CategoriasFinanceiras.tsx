...
import { FolderTree, Plus, Search, X, Save, Loader2, ArrowUp, ArrowDown, LayoutList, LayoutGrid, Trash2 } from 'lucide-react'
...
export default function CategoriasFinanceiras() {
  ...
  const handleBulkDelete = async () => {
    ...
    await supabase.from('fin_categorias_financeiras').delete().in('id', [...selected])
    setSelected(new Set())
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                ...
                <th className="text-left px-4 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-widest hidden md:table-cell cursor-pointer select-none" onClick={() => toggleSort('grupo')}>
            ...
            <div key={c.id} onClick={() => openEdit(c)}
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {c.grupo && <p className="text-[10px] text-slate-400 mt-0.5">{c.grupo.codigo} — {c.grupo.descricao}</p>}
                </div>
              </div>
            </div>
          ))}
