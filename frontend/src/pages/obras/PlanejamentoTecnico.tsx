...
interface Estrutura { id: string; obra_id: string; nome: string; tipo: string | null; peso_ton: number | null; ordem: number }
...
export default function PlanejamentoTecnico() {
  ...
  const obraSel = obraId || obras[0]?.id
  ...
  const [novo, setNovo] = useState({ nome: '', tipo: '', peso: '' })
  ...
  const addEstrutura = useMutation({
    ...
    onSuccess: () => { setNovo({ nome: '', tipo: '', peso: '' }); setAddOpen(false); qc.invalidateQueries({ queryKey: ['obr-estruturas', obraSel] }) },
  })
  ...
  return (
    <div className="space-y-3">
      ...
      <div className={`rounded-2xl ${card} p-3 flex items-center gap-2 flex-wrap`}>
        {portfolios.length > 1 && (
          <select value={portfolioId ?? ''} onChange={e => { setPid(e.target.value); setObraId('') }} className={`${sel} max-w-[200px] truncate`}>
            {portfolios.map(p => <option key={p.id} value={p.id}>{p.nome_obra}</option>)}
          </select>
        )}
        <select value={obraSel ?? ''} onChange={e => setObraId(e.target.value)} className={`${sel} max-w-[320px] truncate`}>
          {obras.map(o => <option key={o.id} value={o.id}>{o.nome}</option>)}
        </select>
        <div className="relative">
          <Search size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400" />
          <input value={qTorre} onChange={e => setQTorre(e.target.value)} placeholder="filtrar torre..." className={`pl-7 pr-3 py-1.5 rounded-lg border text-xs w-36 ${isDark ? 'bg-white/[0.06] border-white/[0.1] text-slate-200' : 'bg-white border-slate-200'}`} />
        </div>
        <div className="flex-1" />
        <span className={`text-[10px] ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>{estruturas.length} estrutura(s)</span>
