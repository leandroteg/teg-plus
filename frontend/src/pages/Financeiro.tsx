...
import {
  DollarSign, TrendingUp, ArrowLeft, Clock,
  BarChart2, CreditCard, RefreshCw, PieChart,
  Landmark, CalendarDays, CheckCircle2,
} from 'lucide-react'
...
const FEATURES = [
  {
    icon: TrendingUp,
    name: 'Fluxo de caixa em tempo real',
    desc: 'Visualização dinâmica de entradas e saídas com projeções futuras',
  },
  {
    icon: CreditCard,
    name: 'Contas a pagar e a receber',
    desc: 'Gestão completa de obrigações financeiras com alertas de vencimento',
  },
  {
    icon: RefreshCw,
    name: 'Integração Omie',
    desc: 'Sincronização automática bidirecional com o ERP Omie',
  },
  {
    icon: Landmark,
    name: 'Conciliação bancária',
    desc: 'Comparação automática entre extratos bancários e lançamentos internos',
  },
  {
    icon: BarChart2,
    name: 'DRE — Demonstrativo de Resultados',
    desc: 'Relatório gerencial de receitas, custos e lucros por período',
  },
  {
    icon: PieChart,
    name: 'Previsão orçamentária',
    desc: 'Budget por centro de custo com acompanhamento de realizado vs. previsto',
  },
  {
    icon: CalendarDays,
    name: 'Programação de pagamentos',
    desc: 'Agenda financeira integrada com aprovação de desembolsos',
  },
  {
    icon: DollarSign,
    name: 'Relatórios fiscais e contábeis',
    desc: 'Exportação de dados para apuração de impostos e escrituração contábil',
  },
]

// ── Timeline ────────────────────────────────────────────────────────────────
const TIMELINE = [
  { quarter: 'Q2 2026', label: 'Integração Omie + Contas a Pagar', status: 'next' },
  { quarter: 'Q3 2026', label: 'Fluxo de Caixa + Conciliação Bancária', status: 'planned' },
  { quarter: 'Q4 2026', label: 'DRE, Budget e Relatórios Fiscais', status: 'planned' },
]
...
export default function Financeiro() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-gradient-to-br from-emerald-600 via-emerald-500 to-green-400 px-6 pt-8 pb-16 shadow-lg">
        <button
          onClick={() => navigate('/')}
          className="flex items-center gap-1.5 text-emerald-100 hover:text-white text-sm
      ...
      <div className="max-w-2xl mx-auto px-4 -mt-8 pb-12 space-y-5">
              ...
              <div key={quarter} className="flex items-start gap-4">
                ...
                <div className="pb-1">
                  <span className={`text-xs font-bold uppercase tracking-wider ${
                    status === 'next' ? 'text-emerald-600' : 'text-slate-400'
                  }`}>
                    {quarter}
                  </span>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl
            border-2 border-emerald-200 text-emerald-700 font-bold text-sm
