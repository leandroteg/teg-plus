    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className={`rounded-2xl shadow-2xl p-6 w-full max-w-md space-y-4 ${!isDark ? 'bg-white border border-slate-200' : 'bg-[#1e293b] border border-white/[0.06]'}`}>
        <div className="flex items-start justify-between">
          <h2 className={`text-lg font-extrabold ${!isDark ? 'text-slate-800' : 'text-white'}`}>Ocorrência de Telemetria</h2>
        ...
        <div>
          <label className={`block text-xs font-bold mb-1 ${!isDark ? 'text-slate-600' : 'text-slate-300'}`}>Observações / Tratativa</label>
          <textarea
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <form onSubmit={handleSubmit} className={`rounded-2xl shadow-2xl p-6 w-full max-w-lg space-y-4 ${!isDark ? 'bg-white border border-slate-200' : 'bg-[#1e293b] border border-white/[0.06]'}`}>
        <div className="flex items-center justify-between">
          <h2 className={`text-lg font-extrabold ${!isDark ? 'text-slate-800' : 'text-white'}`}>Registrar Ocorrência</h2>
          <button
            type="button"
            onClick={onClose}
