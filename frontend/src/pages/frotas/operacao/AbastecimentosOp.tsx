      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {abastecimentos.map(ab => (
            <div key={ab.id} className={`rounded-xl shadow-sm px-4 py-3 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 ${!isDark ? 'bg-white border border-slate-200' : 'bg-[#1e293b] border border-white/[0.06]'} ${ab.desvio_detectado ? 'border-red-500/30' : ''}`}>
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-xs ${!isDark ? 'text-slate-600' : 'text-slate-300'}`}>{ab.posto ?? 'Posto não informado'}</p>
