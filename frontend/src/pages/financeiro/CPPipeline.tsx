...
function NovaSolicitacaoExtraordinariaModal({
  ...
  return (
    // Fecha SÓ pelo X ou Cancelar — clique/arrasto no fundo não fecha (evita perder o formulário)
...
function CPDetailModal({ cp, stageStatus, onClose, onAction, isDark }: {
  ...
  const obraOptions: SelectOption[] = obrasList.map(o => ({ value: o.id, label: o.nome }))
...
function LoteTableRow({
  ...
  // Empresa pagadora. O lote aceita empresas diferentes (pedido do user
  // 04/ago); empresa_id no lote so existe quando TODOS os titulos sao da mesma.
  ...
  const empresaLote = (summary.lote as any).empresa_id
    ...
    : null
  const empresasDosItens = new Set(summary.allItems.map(cp => cp.empresa_id ?? 'sem'))
  const empresaChip = empresaLote
    ? (empresaLote.nome_fantasia || empresaLote.razao_social)
    : empresasDosItens.size > 1
      ? `${empresasDosItens.size} empresas`
      : 'Sem empresa'
  const empresaDefinida = !!empresaLote || empresasDosItens.size > 1
