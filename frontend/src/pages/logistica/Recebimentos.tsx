  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className={`text-xl font-extrabold ${isDark ? 'text-white' : 'text-navy'}`}>Recebimentos</h1>
          <p className={`text-xs mt-0.5 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Confirmação de entrega pelo destinatário</p>
        </div>
      </div>

      {/* SLA Alert */}
      <div className={`rounded-2xl px-4 py-3 flex items-start gap-2.5 ${isDark ? 'bg-blue-500/10 border border-blue-500/20' : 'bg-blue-50 border border-blue-200'}`}>
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {recebimentos.map(r => {
            ...
            return (
              <div key={r.id} className={`rounded-2xl border shadow-sm overflow-hidden ${isDark ? `bg-[#1e293b] ${atrasado ? 'border-amber-500/30' : 'border-white/[0.06]'}` : `bg-white ${atrasado ? 'border-amber-300' : 'border-slate-200'}`}`}>
                <div className="flex items-center gap-3 px-4 py-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${r.status === 'confirmado' ? (isDark ? 'bg-emerald-500/10' : 'bg-emerald-50') : r.status === 'pendente' ? (isDark ? 'bg-amber-500/10' : 'bg-amber-50') : (isDark ? 'bg-red-500/10' : 'bg-red-50')}`}>
                    {r.status === 'confirmado'
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`inline-flex items-center gap-1 rounded-full text-[10px] font-semibold px-2 py-0.5 ${isDark ? 'bg-white/10 text-slate-200' : `${cfg.bg} ${cfg.text}`}`}>
                  ...
                  <div className="shrink-0">
                    {r.status === 'pendente' ? (
                      ...
                      <button
                        ...
                        className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${isDark ? 'bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/20' : 'bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200'}`}
                      >
                ...
                {r.status !== 'pendente' && (r.divergencias || r.confirmado_em) && (
                  <div className={`px-4 py-2.5 space-y-1 ${isDark ? 'border-t border-white/[0.06]' : 'border-t border-slate-50'}`}>
                    {r.confirmado_em && (
                      <p className={`text-[10px] ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
                    ...
                    {r.divergencias && (
                      <p className={`text-[10px] font-medium ${isDark ? 'text-amber-400' : 'text-amber-600'}`}>Divergências: {r.divergencias}</p>
                    )}
                    {r.assinatura_digital && (
                      <p className={`text-[9px] font-mono ${isDark ? 'text-slate-600' : 'text-slate-400'}`}>Assinatura: {r.assinatura_digital}</p>
                    )}
                  </div>
                )}
              </div>
          ...
          <div className={`rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto ${isDark ? 'bg-[#1e293b]' : 'bg-white'}`}>
            ...
            <div className="p-6 space-y-4">
              <div className={`rounded-xl px-3 py-2.5 ${isDark ? 'bg-white/5' : 'bg-slate-50'}`}>
                <p className={`text-xs font-bold ${isDark ? 'text-white' : 'text-slate-700'}`}>{confirmModal.solicitacao?.numero}</p>
                <p className={`text-[10px] ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
          ...
          <div className={`rounded-2xl shadow-2xl w-full max-w-sm ${isDark ? 'bg-[#1e293b]' : 'bg-white'}`}>
            ...
            <div className="p-6 space-y-4">
              {([
                ...
                <div key={k}>
                  <label className={`block text-xs font-bold mb-1.5 ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>{label}</label>
                  <div className="flex gap-1.5">
                    {[1, 2, 3, 4, 5].map(n => (
              ...
              <div>
                <label className={`block text-xs font-bold mb-1 ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>Comentário</label>
