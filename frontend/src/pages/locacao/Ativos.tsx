...
import {
  ...
  Pencil, Save, Loader2, Plus, Sparkles,
} from 'lucide-react'
import { useTheme } from '../../contexts/ThemeContext'
import { useImoveis, useAditivos, useVistorias, useAtualizarImovel, useLimpezas } from '../../hooks/useLocacao'
...
function ImovelDetailModal({ imovel: imovelProp, aditivos, vistorias, onClose, isDark }: {
  ...
  // Limpezas vêm do Portal TEG (loc_limpezas) — buscadas só quando a ficha abre.
  const { data: limpezas = [] } = useLimpezas({ imovel_id: imovel.id })
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={onClose}>
      <div className={`rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto ${bg}`} onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
          ...
          {/* Limpezas registradas no Portal TEG */}
          <div className={`rounded-xl p-4 ${cardBg}`}>
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-2.5 flex items-center gap-1.5">
              <Sparkles size={11} /> Limpezas (Portal)
            </p>
            {limpezas.length === 0 ? (
              <p className={`text-xs ${txtMuted}`}>Nenhuma limpeza registrada para este imóvel.</p>
            ) : (
              <div className="space-y-2">
                {limpezas.slice(0, 12).map(l => {
                  const pend = (l.areas ?? []).filter(a => a.estado === 'pendente')
                  return (
                    <div key={l.id} className={`rounded-lg p-3 border ${isDark ? 'border-white/[0.06] bg-white/[0.02]' : 'border-slate-200 bg-white'}`}>
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        <span className={`text-xs font-bold ${txtMain}`}>
                          {fmtDate(l.data)} · {l.tipo === 'profunda' ? 'Profunda' : l.tipo === 'pos_saida' ? 'Pós-saída' : 'Rotina'}
                        </span>
                        <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold ${
                          pend.length === 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'
                        }`}>
                      ...
                      {l.observacao && (
                        <p className={`text-[11px] mt-1.5 ${txtMain}`}>{l.observacao}</p>
                      )}
                      {(l.fotos ?? []).length > 0 && (
