...
import { LayoutGrid, LogOut, Shield, ShieldCheck, Settings, ChevronLeft, ChevronRight, Menu, X, User, ClipboardList, Plus, HandHelping, CheckSquare, Receipt } from 'lucide-react'
...
export interface NavActionMenuItem {
  ...
  label: string
  description?: string
  tone?: 'amber' | 'emerald' | 'sky' | 'violet' | 'rose' | 'blue' | 'cyan'
  action: () => void
}
...
export default function ModuleLayout({
  ...
  const { perfil, isAdmin, signOut, role, papelGlobal, getPapelForModule, hasModule } = useAuth()
  ...
  function navActionMenuToneClasses(tone: NavActionMenuItem['tone']) {
    switch (tone) {
      ...
      case 'cyan':   return ls ? 'bg-cyan-50 text-cyan-600'     : 'bg-cyan-500/15 text-cyan-300'
    ...
    const dropdownEl = (
      ...
      >
        ...
        <div className="py-1">
          ...
          {/* Mesmo agrupamento do menu da home (ModuloSelector): um único atalho
              para o painel admin, que já lista Usuários/Políticas/Integrações/Dev. */}
          {isAdmin && (
            <>
              <div className={`h-px mx-3 my-1 ${ls ? 'bg-slate-100' : 'bg-white/[0.06]'}`} />
              <button
