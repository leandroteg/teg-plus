...
const config: Record<string, { dot: string; bg: string; text: string; label: string }> = {
  ...
  em_analise_tecnica: { dot: 'bg-fuchsia-500', bg: 'bg-fuchsia-50', text: 'text-fuchsia-700', label: 'Sala Técnica' },
  ...
  devolvida_solicitante: { dot: 'bg-rose-500', bg: 'bg-rose-50',    text: 'text-rose-700',    label: 'Devolvida p/ Correção' },
  // ── Etapa 3: Cotação ───────────────────────────────────────────────────────
...
function StatusBadge({ status, size = 'md', customLabel }: Props) {
