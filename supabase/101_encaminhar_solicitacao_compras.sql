-- 101: ponte CD -> Compras
-- RPC que encaminha o pendente (nao atendido pelo estoque) de uma solicitacao de
-- material para Compras: cria a RC (em_aprovacao) + itens + aprovacao pendente, e
-- marca a solicitacao como encaminhada_compras com o vinculo cmp_requisicao_id.
...
BEGIN
  ...
  -- Aprovacao pendente (alcada nivel 1)
...
GRANT EXECUTE ON FUNCTION public.est_encaminhar_solicitacao_compras(uuid) TO authenticated;
