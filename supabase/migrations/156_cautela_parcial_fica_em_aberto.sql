-- ─────────────────────────────────────────────────────────────────────────────
-- 156_cautela_parcial_fica_em_aberto.sql
--
-- UX: devolução PARCIAL deve manter a cautela em 'em_aberto' (material ainda em
-- poder do colaborador) até devolver tudo, quando vira 'encerrada'. Antes uma
-- parcial jogava a cautela para 'em_devolucao', somindo da visão de ativas.
--
-- Muda só a transição de status na RPC (resto idêntico à mig 155) e migra as
-- cautelas que ficaram em 'em_devolucao' (parciais) de volta para 'em_aberto'.
-- ─────────────────────────────────────────────────────────────────────────────

UPDATE est_cautelas SET status = 'em_aberto' WHERE status = 'em_devolucao';
...
BEGIN
  ...
  -- Parcial mantém 'em_aberto' (material ainda fora); só fecha quando tudo voltar.
  v_status_novo := CASE WHEN v_all_returned THEN 'encerrada' ELSE 'em_aberto' END;
