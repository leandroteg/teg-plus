-- ─────────────────────────────────────────────────────────────────────────────
-- 152_est_concluir_inventario_sem_atualizado_em.sql
--
-- Bug: migrations 142 e 143 criaram est_concluir_inventario com
-- `SET atualizado_em = now()` mas est_inventarios NUNCA teve essa coluna
-- (auditoria mig 121 cobre só cmp_/fin_/con_/ti_). Resultado: chamada
-- estoura 42703 e o status NUNCA vira 'concluido' — saldo nunca ajusta.
--
-- Fix: remove `atualizado_em` dos dois UPDATEs. Resto da RPC fica igual.
-- ─────────────────────────────────────────────────────────────────────────────
