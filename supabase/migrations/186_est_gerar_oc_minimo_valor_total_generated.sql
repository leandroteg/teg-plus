-- ─────────────────────────────────────────────────────────────────────────────
-- 186_est_gerar_oc_minimo_valor_total_generated.sql
--
-- Corrige est_gerar_oc_minimo: cmp_requisicao_itens.valor_total_estimado é
-- coluna GENERATED (quantidade * valor_unitario_estimado) e a RPC tentava
-- inseri-la explicitamente → "cannot insert a non-DEFAULT value into column
-- 'valor_total_estimado'". Vinha assim desde a mig 128 (a coluna virou
-- generated depois). Remove a coluna do INSERT; o resto é idêntico à mig 183.
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- valor_total_estimado NAO entra: coluna GENERATED (qtd * valor_unitario)
