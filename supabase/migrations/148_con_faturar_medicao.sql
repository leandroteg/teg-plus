-- ─────────────────────────────────────────────────────────────────────────────
-- 148_con_faturar_medicao.sql
--
-- Envio de medição (con_medicoes) ao Financeiro:
--   • Contrato RECEITA  → cria 1 fin_contas_receber (status='previsto', natureza='medicao').
--   • Contrato DESPESA  → cria 1 fin_contas_pagar   (status='previsto', natureza='medicao',
--                                                    origem='medicao_contrato').
-- Vencimento = periodo_fim + COALESCE(c.sla_dias_pagamento, 30).
-- Marca a medição como 'faturado' ao concluir.
-- Espelha o padrão de cartao_enviar_fatura_financeiro (146).
-- ─────────────────────────────────────────────────────────────────────────────

ALTER TABLE fin_contas_pagar
  ADD COLUMN IF NOT EXISTS medicao_id uuid
  REFERENCES con_medicoes(id) ON DELETE SET NULL;

ALTER TABLE fin_contas_receber
  ADD COLUMN IF NOT EXISTS medicao_id uuid
  REFERENCES con_medicoes(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_fin_contas_pagar_medicao_id
  ON fin_contas_pagar(medicao_id)
  WHERE medicao_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_fin_contas_receber_medicao_id
  ON fin_contas_receber(medicao_id)
  WHERE medicao_id IS NOT NULL;
...
COMMENT ON FUNCTION public.con_faturar_medicao(uuid) IS
  'Envia medicao aprovada ao Financeiro: cria CP (despesa) ou CR (receita) com status previsto. Marca medicao como faturada. Pula se ja enviada.';

GRANT EXECUTE ON FUNCTION public.con_faturar_medicao(uuid) TO authenticated;
