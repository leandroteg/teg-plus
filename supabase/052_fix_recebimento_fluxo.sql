...
ALTER TABLE public.cmp_recebimentos
  DROP CONSTRAINT IF EXISTS cmp_recebimentos_recebido_por_fkey;

ALTER TABLE public.cmp_recebimentos
  ADD CONSTRAINT cmp_recebimentos_recebido_por_fkey
  FOREIGN KEY (recebido_por) REFERENCES public.sys_perfis(id);
...
BEGIN
  ...
  IF NEW.tipo_destino = 'consumo' THEN
    IF NEW.item_estoque_id IS NULL THEN
      RAISE EXCEPTION 'Item de estoque obrigatorio para recebimento de consumo';
    END IF;
