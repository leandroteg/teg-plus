---
title: "🚗 Frotas Dashboard"
type: painel-bi
tags: [painel, bi, dashboard, frotas]
...
```dataviewjs
const tarefas = dv.pages('"obsidian/Database/Tarefas"').where(t => t.modulo === "frotas");
...
const submodulos=[
  {nome:'Cadastro de Veículos',pct:100,icon:'🚗'},{nome:'Ordens de Serviço (OS)',pct:100,icon:'🔧'},
  {nome:'Checklists Diários',pct:100,icon:'✅'},{nome:'Abastecimentos / Combustível',pct:100,icon:'⛽'},
  {nome:'Telemetria (Simulado)',pct:40,icon:'📡'},{nome:'Telemetria Real (IoT)',pct:0,icon:'📡'},
  {nome:'Controle CNH / Vencimentos',pct:0,icon:'🪪'},{nome:'Custos por Km / TCO',pct:0,icon:'💲'},
  {nome:'Multas e Sinistros',pct:0,icon:'⚠️'},
];
const veiculoStats=[{label:'Caminhões',qty:12,icon:'🚛'},{label:'Pickups',qty:18,icon:'🛻'},{label:'Veículos Leves',qty:8,icon:'🚗'},{label:'Equipamentos',qty:6,icon:'🏗️'}];
const totalVeic=veiculoStats.reduce((s,v)=>s+v.qty,0);
...
<div style="font-family:var(--font-interface);color:var(--text-normal);padding:0 0 20px;">
  ...
  <div style="display:flex;align-items:center;gap:14px;margin-bottom:20px;">
    ...
    <div><div style="font-size:1.4em;font-weight:700;">Módulo Frotas</div>
      <div style="font-size:.78em;color:var(--text-muted);">Veículos · OS Manutenção · Checklists · Abastecimentos · Telemetria</div></div>
  ...
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:12px;">
    ${kpi(submodulos.filter(s=>s.pct===100).length,'Features Prontas',C.green,`de ${submodulos.length} submódulos`,pct(submodulos.filter(s=>s.pct===100).length,submodulos.length))}
    ${kpi(totalVeic,'Veículos Cadastrados',C.blue,'em 6 obras ativas',undefined)}
    ...
    ${kpi(submodulos.filter(s=>s.pct===0).length,'Gaps Pendentes',C.red,'Telemetria IoT, CNH, TCO, Multas',undefined)}
  </div>

  <div style="display:grid;grid-template-columns:3fr 2fr;gap:12px;margin-bottom:12px;">
    ${card(`${secHead('📊 Submódulos')}${submodulos.map(s=>{const col=s.pct===100?C.green:s.pct>=40?C.blue:s.pct>0?C.orange:'#555';return `<div style="margin:7px 0;"><div style="display:flex;justify-content:space-between;margin-bottom:3px;font-size:.8em;"><span>${s.icon} ${s.nome}</span><span style="color:${col};font-weight:600;">${s.pct}%</span></div>${pBar(s.pct,col,7)}</div>`;}).join('')}`)}
    ${card(`${secHead('🚛 Composição da Frota')}${veiculoStats.map(v=>`<div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid rgba(128,128,128,.08);"><span style="font-size:1.4em;">${v.icon}</span><div style="flex:1;"><div style="font-size:.88em;font-weight:500;">${v.label}</div>${pBar(pct(v.qty,totalVeic),C.blue,5)}</div><div style="text-align:right;"><div style="font-size:1.2em;font-weight:700;color:${C.blue};">${v.qty}</div><div style="font-size:.65em;color:var(--text-muted);">${pct(v.qty,totalVeic)}%</div></div></div>`).join('')}
      <div style="margin-top:14px;padding:12px;background:rgba(168,85,247,.06);border-radius:8px;border-left:3px solid ${C.purple};"><div style="font-size:.75em;color:var(--text-muted);margin-bottom:4px;">📡 Próximo</div><div style="font-size:.84em;">Telemetria real via IoT/OBD2 para rastreamento GPS, consumo e alertas de manutenção preventiva.</div></div>`)}
  </div>
